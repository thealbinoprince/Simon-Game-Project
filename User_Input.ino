...
void buttonPress() {
  for (int i = 0; i < difficulty; i++){
     setLED(readButtons());
     playTone(readButtons());
     if (readButtons() != lightSequence[i]){
       mode = 0;
       playTone(4);
       difficulty = 0;
     }
...
int readButtons() {
  while (digitalRead(redButton) == HIGH && digitalRead(yellowButton) == HIGH && digitalRead(blueButton) == HIGH){
    delay(20);
  }
...
void setGame() {
  difficulty = 1; //in order to test random LEDs before finishing the game, change the difficulty here
...
void playTone(int num) {
  switch(num) {
      ...
      case 3:
          ...
          break;
      case 4:
          tone(buzzer, redTone + yellowTone + blueTone, toneTime * 4);
